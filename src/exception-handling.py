...
try:
    ...
    print(
        # f stings for better formatting
...
class FileHasToManyRows(Exception):
    ...
    def __init__(self, number_of_rows):
        ...
        self.message = f"Csv file has too many rows, max rows is 1000 and the file has {
            self.number_of_rows}"
...
try:
    e_commerce_csv_df = pd.read_csv(
        e_commerce_data_fake_path_csv,  encoding='unicode_escape', nrows=1100)
